  ...
  const handleCreateMergeBranch = async () => {
    ...
    try {
      ...
      if (hasLocalWork) {
        ...
        // 读取当前项目根 pom.xml 的 parent.version 作为源版本基线
      ...
      } // end if (hasLocalWork) - first phase

      // ========== 第二阶段：对所有目标分支进行推送和创建合并请求（仅本地仓库） ==========
      if (hasLocalWork) {
      ...
      if (hasRemoteWork) {
        ...
        for (const repoId of selectedRemoteRepos) {
          ...
          // 2. 对每个目标分支执行：创建合并分支 + cherry-pick + push
